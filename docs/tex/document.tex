...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.52\textwidth]{images_tex/pyslam-teaser-rect.png}
  \label{fig:teaser}
\end{figure}
...
\textbf{pySLAM} is an open-source framework designed to accelerate research and development in Visual SLAM (Simultaneous Localization and Mapping)\cite{cadena2017past} and Spatial AI\cite{davison2018futuremapping}. It originated from a practical need identified during the development of a SLAM/Spatial AI course: The  absence of a unified, accessible, and extensible SLAM ecosystem in C++ and Python. The project was driven by several foundational motivations, including the desire to rapidly prototype and evaluate both \textit{classical} and \textit{learning-based} features within complete VO/SLAM pipelines, ensure reproducibility across a wide range of datasets, and significantly lower the entry barrier for newcomers to the field.

A key decision behind pySLAM development was the choice of \textit{Python} as its primary language. Compared to C++, Python enables faster experimentation and benefits from a mature and expanding ecosystem of deep learning tools and research-oriented APIs. This choice aligns with current trends in computer vision and robotics research, where rapid prototyping and flexible experimentation are essential.
...
To address these issues, pySLAM is built on several key design principles. \textit{Modularity} is central: components such as local/global features\cite{tuytelaars2008local,joshi2020recent,ma2021image}, loop closure mechanisms\cite{schubert2023visual}, semantic mapping\cite{georgevichsemantic,chen2025semantic,schmid2024khronos}, and volumetric integration\cite{dong2022ash,freda2023plvs} can be easily swapped or extended. This architecture supports the use of interchangeable baselines, enabling researchers to plug-and-play with different configurations depending on their goals. \textit{Extensibility} is facilitated through a base-class architecture that allows new components to be integrated with minimal friction, making pySLAM a flexible platform for benchmarking and algorithm development.

In terms of usability, pySLAM emphasizes simplicity in the build and deployment process.. It supports a unified Python environment compatible with \textit{conda}, \textit{pyenv}, and \textit{pixi}, and offers a one-command installation process that covers setup and dataset acquisition. Cross-platform compatibility (Linux, macOS, Windows with WSL2) and full source accessibility further enhance its utility for both academic and industrial use.

Another cornerstone of the framework is standardized dataset handling. By providing a unified interface to multiple datasets, pySLAM encourages \textit{reproducibility} and \textit{reusability}. It supports consistent data loading, map state saving/loading, and automatic generation of evaluation metrics -- including final and online trajectory estimation -- thus enabling robust comparison between algorithms.

Functionally, pySLAM supports the full visual SLAM pipeline with monocular, stereo, and RGB-D configurations. It accommodates both traditional local features and deep-learning-based methods, and includes loop closure detection integrated with optimization backends like g2o\cite{g2o_kummerle} and GTSAM\cite{factor_graphs_for_robot_perception}. The framework also supports 3D reconstruction via different voxel grid models, TSDF\cite{dong2022ash} and emerging techniques like Gaussian Splatting\cite{bao20253d}, and integrates deep learning models for depth prediction and semantic scene understanding.

Beyond SLAM, pySLAM also provides a unified \textit{ScenefromViews} architecture for modern feed-forward inference of 3D scenes from multiple views. The module supports both pairwise models (DUSt3R\cite{wang2023dust3r}, MASt3R\cite{leroy2024grounding}) that process image pairs and perform global alignment, as well as multi-view models (Depth Anything V3\cite{depthanything3}, MV-DUSt3R\cite{tang2025mv}, VGGT\cite{wang2025vggt}, robust VGGT\cite{vggt_robust}, and Fast3R\cite{yang2025fast3r}) that process multiple views simultaneously in a single forward pass. This modular design enables flexible integration of diverse 3D reconstruction techniques while maintaining a consistent API across different model architectures.
...
The objective of this document is to present the pySLAM framework, its main features, and usage\footnote{You may find an updated version of this document at:\\ \href{https://github.com/luigifreda/pyslam/blob/master/docs/tex/document.pdf}{github.com/luigifreda/pyslam/blob/master/docs/tex/document.pdf}}. 


\section{Main Features}

\textbf{pySLAM} provides a python implementation of a \emph{Visual SLAM} pipeline that supports \textbf{monocular}, \textbf{stereo} and
\textbf{RGBD} cameras. It offers the following \textbf{features} in a single python environment: 
...
\begin{itemize}
    ...
    aggregators}} such as visual Bag of Words (BoW\cite{galvez2012bags}, iBow\cite{garcia2018ibow}), Vector of Locally
    Aggregated Descriptors (VLAD\cite{jegou2010aggregating}), and modern
    ...
    reconstruction. This can use \textbf{TSDF}\cite{dong2022ash} with voxel hashing or
    incremental \textbf{Gaussian Splatting}\cite{matsuki2023gaussian,kerbl20233d}.
    
    \item Integration of
    \textbf{\protect\hyperlink{depth-prediction}{depth prediction models}}
    within the SLAM pipeline. These include DepthPro\cite{depthpro2023}, DepthAnythingV2\cite{depthanythingv2_2024},
    RAFT-Stereo\cite{teed2021raft}, CREStereo\cite{li2022cres}, MASt3R\cite{master}, MVDUSt3R\cite{mvduster}, etc.

    \item A suite of segmentation models for
    \textbf{\protect\hyperlink{semantic-mapping}{semantic understanding}} of
    the scene, such as DeepLabv3\cite{chen2017rethinking}, Segformer\cite{xie2021segformer}, CLIP\cite{lin2023clip}, EOV-Seg\cite{eov_seg}, DETIC\cite{detic}, ODISE\cite{odise}, RFDETR\cite{rfdetr}, and YOLO\cite{redmon2016yolo,redmon2017yolo9000,redmon2018yolov3,bochkovskiy2020yolov4,jocher2020yolov5, wang2022yolov7, jocher2023yolov8, sapkota2025ultralytics}.
    
    \item A \textbf{\protect\hyperlink{scene-from-views}{Scene from Views}} module providing a unified interface for feed-forward inference of 3D scenes from multiple views, supporting models such as DUSt3R\cite{wang2023dust3r}, MASt3R\cite{leroy2024grounding}, Depth Anything V3\cite{depthanything3}, MV-DUSt3R\cite{tang2025mv}, VGGT\cite{wang2025vggt}, and Fast3R\cite{yang2025fast3r}.
    
    \item A modular sparse SLAM \textbf{\protect\hyperlink{cpp-core}{Core}}, implemented both in Python and C++ (with custom pybind11 bindings), allowing users to switch between high-performance/speed and high-flexibility modes.
    
    \item Additional tools for VO (Visual Odometry) and SLAM, with built-in
    support for both \textbf{g2o}\cite{g2o_kummerle} and \textbf{GTSAM}\cite{factor_graphs_for_robot_perception}, along with custom
    Python bindings for features not available in the original libraries

    \item Built-in support for over \textbf{\protect\hyperlink{datasets}{10 dataset
    types}}. The list is currently growing.
\end{itemize}

pySLAM serves as a flexible baseline framework to experiment with VO/SLAM
...
prediction}}, \emph{\protect\hyperlink{semantic-mapping}{semantic
mapping}}, \emph{\protect\hyperlink{scene-from-views}{scene from views}}, and \emph{\protect\hyperlink{cpp-core}{C++ core}}. It allows to explore, prototype and develop VO/SLAM
...
A convenient entry point is the following collection of \textbf{main scripts}:
\begin{itemize}
\item 
...
\item
  ...
  \item
  \texttt{main\_scene\_from\_views.py} infers 3D scenes from multiple
  images using models like Mast3r, VGGT, DepthFromAnythingV3, and Fast3R (see
  \protect\hyperlink{scene-from-views}{here}).
\end{itemize}
...
\subsection{Main SLAM System Components}
...
The diagram in Fig.~\ref{Fig:FeatureMatcher} illustrates the architecture of the \textit{Feature Matcher} module. At its core is the \texttt{feature\_matcher\_factory}, which instantiates matchers based on a specified \texttt{matcher\_type}, such as \texttt{BF}, \texttt{FLANN}, \texttt{XFEAT}\cite{barroso2024xfeat}, \texttt{LIGHTGLUE}\cite{lindenberger2023lightglue}, and \texttt{LOFTR}\cite{sun2021loftr}. Each of these creates a corresponding matcher implementation (e.g., \texttt{BfFeatureMatcher}, \texttt{FlannFeatureMatcher}, etc.), all inheriting from a common \texttt{FeatureMatcher} interface.
...
The diagram in Fig.~\ref{Fig:LoopDetector} shows the architecture of the \textit{Loop Detector} component. A central \texttt{loop\_detector\_factory} instantiates loop detectors based on the selected \texttt{global\_descriptor\_type}, which may include traditional descriptors (e.g., \texttt{DBOW2}\cite{galvez2012bags}, \texttt{VLAD}\cite{jegou2010aggregating}, \texttt{IBOW}\cite{garcia2018ibow}) or deep learning-based embeddings (e.g., \texttt{NetVLAD}\cite{arandjelovic2016netvlad}, \texttt{CosPlace}\cite{berton2023cosplace}, \texttt{EigenPlaces}\cite{berton2023eigenplaces}).
...
The diagram in Fig.~\ref{Fig:DepthEstimator} illustrates the architecture of the \textit{Depth Estimator} module. A central \texttt{depth\_estimator\_factory} creates instances of various depth estimation backends based on the selected \texttt{depth\_estimator\_type}, including both traditional and learning-based methods such as \texttt{DEPTH\_SGBM}\cite{hirschmuller2007stereo}, \texttt{DEPTH\_RAFT\_STEREO}\cite{teed2021raft}, \texttt{DEPTH\_ANYTHING\_V2}\cite{depthanythingv2_2024}, \texttt{DEPTH\_MAST3R}\cite{master}, and \texttt{DEPTH\_MVDUST3R}\cite{mvduster}.

Each estimator type instantiates a corresponding implementation (e.g., \texttt{DepthEstimatorSgbm}, \texttt{DepthEstimator\-Crestereo\-Megengine}, etc.), all inheriting from a common \texttt{DepthEstimator} interface. This base class encapsulates shared dependencies such as the \texttt{camera}, \texttt{device}, and \texttt{model} components, allowing for modular integration of heterogeneous depth estimation techniques across stereo, monocular, and multi-view pipelines.
...
The diagram in Fig.~\ref{Fig:VolumetricIntegrator} illustrates the structure of the \textit{Volumetric Integrator} module. At its core, the \texttt{volumetric\_integrator\_factory} generates specific volumetric integrator instances based on the selected \texttt{volumetric\_integrator\_type}, such as \texttt{VOXEL\_GRID}, \texttt{VOXEL\_SEMANTIC\_GRID}, \texttt{VOXEL\_SEMANTIC\_PROBABILISTIC\_GRID}, \texttt{TSDF}\cite{dong2022ash}, and \texttt{GAUSSIAN\_SPLATTING}\cite{matsuki2023gaussian,kerbl20233d}.

Each type instantiates a dedicated implementation (e.g., \texttt{VolumetricIntegratorVoxelGrid}, \texttt{VolumetricIntegrator\-Voxel\-Semantic\-Grid}, \texttt{VolumetricIntegratorTSDF}, \texttt{VolumetricIntegrator\-Gaussian\-Splatting}), which inherits from a common \texttt{VolumetricIntegratorBase}. This base class encapsulates key components including the \texttt{camera}, a \texttt{keyframe\_queue}, and the \texttt{volume}, enabling flexible integration of various 3D reconstruction methods within a unified pipeline.
...
The diagram in Fig.~\ref{Fig:SemanticMapping} outlines the architecture of the \textit{Semantic Mapping} module. At its core is the \texttt{semantic\_mapping\_factory}, which creates semantic mapping instances according to the selected \texttt{semantic\_mapping\_type}. Currently, the supported type is \texttt{DENSE}, which instantiates either the \texttt{SemanticMappingDense} class or the \texttt{SemanticMappingDenseProcess} class if
\begin{scriptsize}
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Parameters.kSemanticMappingMoveSemanticSegmentationToSeparateProcess=True}.
\end{Highlighting}
\end{Shaded}
\end{scriptsize}
Both these classes extend \texttt{SemanticMappingDenseBase} and runs asynchronously in a dedicated thread to process keyframes as they become available. In particular, the \texttt{SemanticMappingDenseProcess} class offloads semantic segmentation inference to a separate process, thereby making better use of CPU core parallelism.

Both \texttt{SemanticMappingDense} and \texttt{SemanticMappingDenseProcess} integrate semantic information into the SLAM map by leveraging per-frame predictions from a semantic segmentation model. The segmentation model is instantiated via the \texttt{semantic\_segmentation\_factory}, based on the selected \texttt{semantic\_segmentation\_type}. Supported segmentation backends include \texttt{DEEPLABV3}\cite{chen2017rethinking}, \texttt{SEGFORMER}\cite{xie2021segformer}, \texttt{CLIP}\cite{lin2023clip}, \texttt{EOV\_SEG}\cite{eov_seg}, \texttt{DETIC}\cite{detic}, \texttt{ODISE}\cite{odise}, \texttt{YOLO}\cite{redmon2016yolo,redmon2017yolo9000,redmon2018yolov3,bochkovskiy2020yolov4,jocher2020yolov5, wang2022yolov7, jocher2023yolov8, sapkota2025ultralytics}, and \texttt{RFDETR}\cite{rfdetr}, each of which corresponds to a dedicated class (\texttt{SemanticSegmentationDeepLabV3}, \texttt{SemanticSegmentationSegformer}, \texttt{SemanticSegmentationCLIP}, \texttt{SemanticSegmentationEovSeg}, \texttt{Semantic\-Segmentation\-Detic}, \texttt{Semantic\-Segmentation\-Odise}), \texttt{Semantic\-Segmentation\-RfDetr}), \texttt{Semantic\-Segmentation\-Yolo}), all inheriting from the shared \texttt{SemanticSegmentationBase}.
...
This modular design decouples semantic segmentation from mapping logic, enabling flexible combinations of segmentation models, datasets (e.g., \texttt{NYU40}\cite{Silberman:ECCV12}, \texttt{Cityscapes}\cite{Cityscapes}), and fusion strategies. It also supports customization via configuration files or programmatic APIs for dataset-specific tuning or deployment.

The Section~\ref{semantic-mapping-and-image-segmentation} provides a list of supported semantic segmentation methods.
A more in-depth presentation of the semantic mapping module is provided in~\cite{pyslamSemantic2025}.

\hypertarget{scene-from-views}{%
\subsection{3D Scene from Views}\label{scene-from-views}}

\begin{figure}[!ht]
\begin{center}
    \includegraphics[width=\textwidth]{./images/scene_from_views.png}
\end{center}
\caption{\textit{SceneFromViews} architecture.}
\label{Fig:SceneFromViews}
\end{figure}

The diagram in Fig.~\ref{Fig:SceneFromViews} illustrates the architecture of the \textit{Scene from Views} module, which provides a unified interface for feed-forward 3D scene reconstruction from multiple views. At its core, the \texttt{scene\_from\_views\_factory} instantiates specific reconstruction models based on the selected \texttt{SceneFromViewsType}, such as \texttt{DUST3R}\cite{wang2023dust3r}, \texttt{MAST3R}\cite{leroy2024grounding}, \texttt{DEPTH\_ANYTHING\_V3}\cite{depthanything3}, \texttt{MVDUST3R}\cite{tang2025mv}, \texttt{VGGT}\cite{wang2025vggt}, \texttt{VGGT\_ROBUST}\cite{vggt_robust}, and \texttt{FAST3R}\cite{yang2025fast3r}.

Each type creates a corresponding implementation (e.g., \texttt{SceneFromViewsDust3r}, \texttt{SceneFromViewsMast3r}, \texttt{SceneFromViewsDepthAnythingV3}, \texttt{SceneFromViewsMvdust3r}, \texttt{SceneFromViewsVggt}, \texttt{SceneFromViewsVggtRobust}, \texttt{SceneFromViewsFast3r}), all inheriting from a common \texttt{SceneFromViewsBase}. This base class implements a unified three-step reconstruction pipeline: \texttt{preprocess\_images()} prepares input images for the specific model, \texttt{infer()} runs model inference, and \texttt{postprocess\_results()} converts raw model output to a standardized \texttt{SceneFromViewsResult} format containing merged point clouds, meshes, camera poses, and optional depth maps or intrinsics.

The module supports both pairwise models (\texttt{DUSt3R}\cite{wang2023dust3r}, \texttt{MASt3R}\cite{leroy2024grounding}) that are feed-forward per pair and then run global alignment, as well as multi-view models (\texttt{DEPTH\_ANYTHING\_V3}\cite{depthanything3}, \texttt{MV-DUSt3R}\cite{tang2025mv}, \texttt{VGGT}\cite{wang2025vggt}, \texttt{Robust VGGT}\cite{vggt_robust}, \texttt{FAST3R}\cite{yang2025fast3r}) that process multiple views simultaneously in a single forward pass. This modular design enables flexible integration of diverse 3D reconstruction techniques while maintaining a consistent API across different model architectures.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#pixi shell           # If you use pixi, this activates the pyslam environment. }
...
If you are using \texttt{pixi} then just run \texttt{pixi\ shell} to
activate the \texttt{pyslam} environment. The file \href{https://github.com/luigifreda/pyslam/blob/master/config.yaml}{config.yaml} can be used as a unique
entry-point to configure the system and its global configuration
parameters contained in
\href{https://github.com/luigifreda/pyslam/blob/master/pyslam/config_parameters.py}{pyslam/config\_parameters.py}. 
\begin{itemize}
\item To \textbf{enable the C++ sparse-SLAM core}, set \texttt{USE\_CPP\_CORE\ =\ True} in the file \texttt{config\_parameters.py}.
\item Further information on how to configure pySLAM are provided \protect\hyperlink{selecting-a-dataset-and-different-configuration-parameters}{here}.
\end{itemize}
Further
...
By default, the script processes a
...
stop \texttt{main\_vo.py} by clicking on one of them and
...
video (available in the folder \texttt{data/videos}) by using its
corresponding camera calibration file (available in the folder
\texttt{settings}). You can stop it by clicking on one of the
...
The file \href{https://github.com/luigifreda/pyslam/blob/master/config.yaml}{config.yaml} can be used as a unique
...
you need to update the file \href{https://github.com/luigifreda/pyslam/blob/master/config.yaml}{config.yaml}:
\begin{itemize}
\item Select
...
the files \texttt{pyslam/io/ground\_truth.py} and
\texttt{pyslam/io/convert\_groundtruth\_to\_simple.py}).
\end{itemize}

You can use the section \texttt{GLOBAL\_PARAMETERS} of the file
\href{https://github.com/luigifreda/pyslam/blob/master/config.yaml}{config.yaml} to override the global configuration
parameters set in \href{https://github.com/luigifreda/pyslam/blob/master/pyslam/config_parameters.py}{pyslam/config\_parameters.py}.
...
\href{https://github.com/luigifreda/pyslam/blob/master/test/cv/test_feature_manager.py}{test/cv/test\_feature\_manager.py}
too.
...
\texttt{pyslam/config\_parameters.py}. Different configuration options
\texttt{LoopDetectorConfigs} can be found in
\href{https://github.com/luigifreda/pyslam/blob/master/pyslam/loop_closing/loop_detector_configs.py}{pyslam/loop\_closing/loop\_detector\_configs.py}: Code comments provide additional useful details.

One can start experimenting with loop closing methods by using the
examples in \texttt{test/loopclosing}. The example
\href{https://github.com/luigifreda/pyslam/blob/master/test/loopclosing/test_loop_detector.py}{test/loopclosing/test\_loop\_detector.py}
...
automatically downloaded into the \texttt{data} folder (see
\href{https://github.com/luigifreda/pyslam/blob/master/pyslam/loop_closing/loop_detector_configs.py}{pyslam/loop\_closing/loop\_detector\_configs.py}).
...
\href{https://github.com/luigifreda/pyslam/blob/master/pyslam/loop_closing/loop_detector_vocabulary.py}{pyslam/loop\_closing/loop\_detector\_vocabulary.py} and correspondingly
create a new loop detector configuration in
\href{https://github.com/luigifreda/pyslam/blob/master/pyslam/loop_closing/loop_detector_configs.py}{pyslam/loop\_closing/loop\_detector\_configs.py} that uses it.
...
\texttt{EigenPlaces}, and \texttt{Megaloc} directly extract their specific \textbf{global
...
\paragraph{Verify your loop detection configuration and verify
vocabulary
...
available in \href{https://github.com/luigifreda/pyslam/blob/master/pyslam/loop_closing/loop_detector_configs.py}{pyslam/loop\_closing/loop\_detector\_configs.py}). 
...
See the file \href{https://github.com/luigifreda/pyslam/blob/master/pyslam/loop_closing/loop_detector_configs.py}{pyslam/loop\_closing/loop\_detector\_configs.py} for further
details.
...
\texttt{kDoVolumetricIntegration=True} and selecting your preferred
method \texttt{kVolumetricIntegrationType} in
\texttt{pyslam/config\_parameters.py}. At present, the following methods
are available: 
\begin{itemize}
\item \texttt{VOXEL\_GRID}, \texttt{VOXEL\_SEMANTIC\_GRID},
\texttt{VOXEL\_SEMANTIC\_PROBABILISTIC\_GRID}: Voxel grid
implementations that leverage parallel spatial hashing, supporting both
direct voxel hashing and indirect voxel-block hashing strategies.
Parallel execution is managed using TBB, and the design accommodates
both simple and semantic voxels. 
\item \texttt{TSDF} (Truncated Signed
Distance Function): It is able to return in output either a pointcloud
or a mesh. 
\item  \texttt{GAUSSIAN\_SPLATTING} (Incremental Gaussian
splatting)\href{https://github.com/luigifreda/pyslam/blob/master/pyslam/dense/volumetric_integrator_factory.py}{pyslam/dense/volumetric\_integrator\_factory.py}). 
\end{itemize}

Note that you need CUDA in order to run \texttt{GAUSSIAN\_SPLATTING} method.
...
To obtain a mesh as output, set
\texttt{kVolumetricIntegrationTsdfExtractMesh=True} in
...
saved sparse map and perform dense reconstruction by using its posed
...
The directory \texttt{<gs\_checkpoint\_path>} is expected to have the following structure:\\
\begin{scriptsize}
\begin{Shaded}
\begin{Highlighting}[]
...
policy, a new keyframe is generated only if its FOV center lies beyond a
predefined distance from the nearest existing keyframe's FOV center. You
can enable this policy by setting the following parameters in the yaml
setting:\\
...
or by setting the following parameters in
\texttt{config\_parameters.py}:

\begin{scriptsize}
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{kUseFovCentersBasedKfGeneration }\OperatorTok{=} \VariableTok{False}  \CommentTok{# Use FOV centers based keyframe generation;}
\FunctionTok{}\AttributeTok{ }\DecValTok{}\AttributeTok{  }\CommentTok{                                       # not considered if KeyFrame.useFovCentersBasedGeneration is set in yaml}
\NormalTok{kMaxFovCentersDistanceForKfGeneration }\OperatorTok{=} \FloatTok{0.2}  \CommentTok{# [m] Maximum distance between FOV centers for keyframe generation;}
\FunctionTok{}\AttributeTok{ }\DecValTok{}\AttributeTok{  }\CommentTok{                                       # not considered if not considered if KeyFrame.maxFovCentersDistance is set in yaml}
\end{Highlighting}
\end{Shaded}
\end{scriptsize}
...
\texttt{pyslam/config\_parameters.py}. 
\item \textbf{Front-end}: Depth prediction
can be enabled in the front-end by setting the parameter
\texttt{kUseDepthEstimator\allowbreak InFrontEnd} in \texttt{pyslam/config\_parameters.py}.
...
front-end (to emulate an RGBD sensor). 
\item Depth inference may be
very slow (for instance, with DepthPro it takes \textasciitilde{}1s per
image on a typical machine). Therefore, the resulting volumetric reconstruction
pipeline may be very slow.
\end{itemize}

Refer to the file
\texttt{pyslam/depth\_estimation/depth\_estimator\_factory.py} for further
...
\hypertarget{semantic-mapping-and-image-segmentation}{%
\subsubsection{Semantic mapping and Image
Segmentation}\label{semantic-mapping-and-image-segmentation}}

The sparse semantic mapping pipeline can be enabled by setting the
parameter \texttt{kDoSparseSemanticMappingAndSegmentation=True} in
\texttt{pyslam/config\_parameters.py}. You can configure the semantic
mapping module by mofifying the default segmentation models assigned to
each dataset in \texttt{pyslam/semantics/semantic\_mapping\_configs.py}.
Otherwise, you can override the used segmentation model by setting
\texttt{kSemanticSegmentationType} in \texttt{config\_parameters.py}.

\hypertarget{sparse-semantic-mapping}{%
\paragraph{Sparse Semantic Mapping}\label{sparse-semantic-mapping}}

Different semantic mapping methods are available (see
\href{https://github.com/luigifreda/pyslam/blob/master/docs/semantics.md}{here} for further details). Currently, we
support semantic mapping using \textbf{dense semantic segmentation}. 
\begin{itemize}
...
\item \texttt{EOV\_SEG}: open-vocabulary panoptic segmentation model.
\item \texttt{DETIC}: open-vocabulary object detection and segmentation model.
\item \texttt{ODISE}: open-vocabulary diffusion-based panoptic segmentation model.
\item \texttt{RFDETR}: real-time transformer architecture for object detection and instance segmentation (built on a DINOv2 vision transformer backbone). 
\item \texttt{YOLO}: built on years of foundational research in computer vision and AI. 
\end{itemize}

\textbf{Panoptic/instance segmentation notes}:
\begin{itemize}
\item \texttt{DETIC}: object detection-based. Each detection yields a distinct instance ID.
\item \texttt{ODISE}: panoptic segmentation model. Instances are derived from panoptic segments via the \texttt{isthing} flag and can merge multiple objects of the same category.
\item \texttt{EOV\_SEG}: similar to \texttt{ODISE}, instances are derived from panoptic segments and may group nearby objects of the same category.
\end{itemize}

\textbf{Semantic features} are assigned to \textbf{keypoints} on the
image and fused into map points. The semantic features can be:
\begin{itemize}
...
\hypertarget{volumetric-semantic-mapping}{%
\paragraph{Volumetric Semantic
mapping}\label{volumetric-semantic-mapping}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{kDoSparseSemanticMappingAndSegmentation}\OperatorTok{=}\VariableTok{True} \CommentTok{#  enable sparse mapping and segmentation}
\NormalTok{kDoVolumetricIntegration }\OperatorTok{=} \VariableTok{True} \CommentTok{# enable volumetric integration}
\NormalTok{kVolumetricIntegrationType }\OperatorTok{=} \StringTok{"VOXEL_SEMANTIC_PROBABILISTIC_GRID"} \CommentTok{# use semantic volumetric models }
\FunctionTok{}\AttributeTok{ }\DecValTok{}\AttributeTok{  }\CommentTok{                                                      # like VOXEL_SEMANTIC_PROBABILISTIC_GRID and VOXEL_SEMANTIC_GRID}
\end{Highlighting}
\end{Shaded}
\end{scriptsize}
...
\begin{itemize}
\item \texttt{VOXEL\_SEMANTIC\_GRID}: confidence-counter fusion (majority voting).
\item \texttt{VOXEL\_SEMANTIC\_PROBABILISTIC\_GRID}: probabilistic fusion, with per-class distributions fused in log-space.
\end{itemize}

Configuration notes:
\begin{itemize}
\item If \texttt{kDoSparseSemanticMappingAndSegmentation=True} and \texttt{kVolumetricIntegrationType="VOXEL\_GRID"}, the factory warns and automatically switches to \texttt{VOXEL\_SEMANTIC\_PROBABILISTIC\_GRID} to enable semantic fusion.
\item If \texttt{kDoSparseSemanticMappingAndSegmentation=True} with a non-semantic volumetric integrator (e.g., \texttt{TSDF}, \texttt{GAUSSIAN\_SPLATTING}), a warning is emitted and semantic integration is skipped.
\end{itemize}

If \texttt{kVolumetricSemanticIntegrationUseInstanceIds=True}, the volumetric integrator uses 2D instance IDs (from panoptic/instance segmentation backends) to build \textbf{3D object segments}. Object IDs are assigned via voting across observations and filtered by \texttt{kVolumetricSemanticIntegrationMinVoteRatio} and \texttt{kVolumetricSemanticIntegrationMinVotes}. This enables:
\begin{itemize}
\item Consistent 3D object grouping across views.
\item Per-object colorization of the volumetric map.
\item Improved object identity stability over time.
\end{itemize}

When volumetric semantic mapping is enabled, use the following GUI buttons:
\begin{itemize}
\item \texttt{Color Semantics}: class/label color maps on both the 3D sparse map and the volumetric map.
\item \texttt{Objects}: per-object color map on the volumetric map (requires instance IDs).
\item \texttt{Draw Object BBs}: bounding boxes for detected 3D object segments.
\end{itemize}

Known limitations in volumetric semantic mapping:
\begin{itemize}
\item \textit{Resolution limits}: a fixed voxel size may blur thin structures and small objects.
\item \textit{Pose drift sensitivity}: accumulated pose errors can smear semantic labels over time.
\item \textit{Instance ID dependence}: inconsistent or noisy 2D instance IDs may lead to fragmented or unstable 3D objects.
\item \textit{Class imbalance bias}: frequently observed classes may dominate voting/probabilistic fusion.
\item \textit{Temporal class inconsistency}: label flickering can fragment voxel labeling over time.
\item \textit{Occlusions and visibility gaps}: poorly observed regions may remain unlabeled or misclassified.
\end{itemize}

Further information about the \textbf{volumetric integration models and
SW architecture} are available \href{https://github.com/luigifreda/pyslam/blob/mastercpp/volumetric/README.md}{here}.

%\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{cpp-core}{%
\subsection{C++ Core for Sparse SLAM}\label{cpp-core}}

The system provides a modular \textbf{sparse-SLAM core}, implemented in \textbf{both Python and C++} (with custom pybind11 bindings), allowing users to switch between high-performance/speed and high-flexibility modes. The C++ core reimplements the sparse SLAM initially implemented in Python, exposing core SLAM classes (frames, keyframes, map points, maps, cameras, optimizers, tracking, and local mapping) to Python via pybind11.
...
To enable the C++ sparse-SLAM core, set \texttt{USE\_CPP\_CORE = True} in \texttt{pyslam/config\_parameters.py}. The module is currently \textbf{under active development}.

The codebase is organized into the following categories:
\begin{itemize}
\item \textbf{Core SLAM Classes}: Main C++ implementations (\texttt{frame.cpp/h}, \texttt{keyframe.cpp/h}, \texttt{map\_point.cpp/h}, \texttt{map.cpp/h}, \texttt{camera.cpp/h}, \texttt{camera\_pose.cpp/h}, \texttt{optimizer\_g2o.cpp/h}, \texttt{tracking\_core.cpp/h}, \texttt{local\_mapping\_core.cpp/h}).
\item \textbf{Serialization}: JSON and NumPy serialization modules for data persistence and Python interop.
\item \textbf{Python Bindings}: pybind11 module definitions that expose C++ classes to Python.
\item \textbf{Type Casters}: pybind11 type casters for custom type conversions (OpenCV, Eigen, JSON, dictionary types).
\item \textbf{Utilities}: Helper functions for geometry, features, descriptors, image processing, NumPy/Eigen operations, and serialization.
\item \textbf{Tests}: C++ unit tests and Python integration tests.
\end{itemize}

The C++ core adopts a streamlined design philosophy:
\begin{itemize}
\item \textbf{All core data resides in C++}: Python serves purely as an interface layer.
\item \textbf{Direct pybind11 exposure}: Python objects are lightweight views of underlying C++ objects.
\item \textbf{Automatic zero-copy}: pybind11 automatically shares NumPy array memory with C++ when possible.
\item \textbf{RAII-based ownership}: C++ smart pointers manage object lifetimes safely and efficiently.
\end{itemize}
...
\ExtensionTok{SYSTEM_STATE}\NormalTok{:}
  \ExtensionTok{folder_path}\NormalTok{: results/slam_state   }\CommentTok{# default folder path (relative to repository root) where the}
  \ExtensionTok{           }\NormalTok{                       }\CommentTok{# system state is saved or reloaded}
\end{Highlighting}
\end{Shaded}
\end{scriptsize}
\end{itemize}
...
front-end, and backend configurations. Reloading a saved map replaces
...
Estimated trajectories can be saved in three formats:
...
\item
  ...
  each front-end iteration for the current frame.
\item
  ...
  multiple times by LBA windows that include it, as well as by PGO and GBA during
  loop closures.
\end{itemize}
...
graph optimization, with \texttt{g2o} set as the default engine. You can enable \texttt{gtsam} by setting to \texttt{True} the
...
bindings for features not available in the original gtsam framework. See
\href{https://github.com/luigifreda/pyslam/blob/master/thirdparty/gtsam_factors/README.md}{here} for further details.
...
\end{codeblock}
Press \texttt{CTRL+A} followed by \texttt{CTRL+Q} to exit from
...
Evaluation results are presented in the following subsections. For convenience, they are also available at this \href{https://github.com/luigifreda/pyslam/blob/master/docs/evaluations/evaluations.md}{link}. The evaluation uses commit \textit{f7c9a13} with the following presets: \texttt{baseline}, \texttt{root\_sift}, and \texttt{superpoint}, which correspond to local feature extractors \texttt{ORB2}, \texttt{root\_SIFT}, and \texttt{SuperPoint}, respectively. 


\hypertarget{tum-evaluation}{%
\subsubsection{TUM Evaluation}\label{tum-evaluation}}

\begin{minipage}{\textwidth}
\noindent
\captionsetup{type=table}
\captionof{table}{TUM: Table RMSE}\label{tab:tum_table_rmse}

\fontsize{7pt}{8pt}\selectfont

\begin{tabularx}{\linewidth}{ >{\RaggedRight\arraybackslash}p{ 8.0cm } >{\RaggedRight\arraybackslash}p{ 1.0cm } >{\RaggedRight\arraybackslash}p{ 1.25cm } >{\RaggedRight\arraybackslash}p{ 1.5cm }  }\toprule
...
\noindent
\captionsetup{type=table}
\captionof{table}{TUM: Table Max }\label{tab:tum_table_max}
...
\noindent
\captionsetup{type=table}
\captionof{table}{TUM: Table Percent Lost }\label{tab:tum_table_percent_lost}
...
\hypertarget{euroc-evaluation}{%
\subsubsection{EUROC Evaluation}\label{euroc-evaluation}}

\begin{minipage}{\textwidth}
\noindent
\captionsetup{type=table}
\captionof{table}{EUROC: Table RMSE}\label{tab:euroc_table_rmse}
...
\noindent
\captionsetup{type=table}
\captionof{table}{EUROC: Table Max }\label{tab:euroc_table_max}
...
\noindent
\captionsetup{type=table}
\captionof{table}{EUROC: Table Percent Lost }\label{tab:euroc_table_percent_lost}
...
\hypertarget{kitti-evaluation}{%
\subsubsection{KITTI Evaluation}\label{kitti-evaluation}}


\begin{minipage}{\textwidth}
\noindent
\captionsetup{type=table}
\captionof{table}{KITTI: Table RMSE}\label{tab:kitt_table_rmse}
...
\noindent
\captionsetup{type=table}
\captionof{table}{KITTI: Table Max }\label{tab:kitt_table_max}
...
\noindent
\captionsetup{type=table}
\captionof{table}{KITTI: Table Percent Lost }\label{tab:kitt_table_percent_lost}
...
\hypertarget{scene-from-views-usage}{%
\subsection{Scene from Views}\label{scene-from-views-usage}}

The \textit{Scene from Views} module provides a unified interface for feed-forward 3D scene reconstruction from multiple
views, following a modular factory pattern architecture. The pipeline
provides a consistent API across different reconstruction models while
maintaining model-specific optimizations. See the main script
\texttt{main\_scene\_from\_views.py}.

\begin{itemize}
\tightlist
\item
  Feed-forward multi-view reconstruction (poses + fused 3D geometry
  directly from images): \texttt{SceneFromViewsDust3r},
  \texttt{SceneFromViewsMast3r}, \texttt{SceneFromViewsMvdust3r},
  \texttt{SceneFromViewsVggt}, \texttt{SceneFromViewsVggtRobust}, \texttt{SceneFromViewsFast3r}.
\item
  Single-view depth-first pipeline with optional poses/intrinsics:
  \texttt{SceneFromViewsDepthAnythingV3}
\item
  Global alignment optimization stage for merging views:
  \texttt{SceneFromViewsDust3r} (dense alignment) and
  \texttt{SceneFromViewsMast3r} (sparse alignment variant)
\item
  Robust view filtering / outlier rejection:
  \texttt{SceneFromViewsVggtRobust} (anchor-based attention + cosine
  scoring that discards low-confidence views before reconstruction)
\end{itemize}

Note that \texttt{DUSt3R} and \texttt{MASt3R} are pairwise models: they
take two images at a time. They are feed-forward per pair; multi-view
reconstruction is then obtained by global alignment / optimization over
all pairwise pointmaps. \texttt{MV-DUSt3R}, \texttt{VGGT}, and
\texttt{Fast3R} are multi-view models that process all images
simultaneously in a single forward pass.

All models return standardized \texttt{SceneFromViewsResult} output
containing point clouds, meshes, camera poses, depth maps, and
intrinsics in a consistent format. The factory pattern allows easy
switching between models while maintaining the same interface.

Further details \href{https://github.com/luigifreda/pyslam/blob/master/pyslam/scene_from_views/README.md}{here}.
...
\begin{itemize}
    ...
    \item \href{https://github.com/raulmur/ORB_SLAM2}{ORB2} (improvements of ORB-SLAM2\cite{ORB_SLAM2} to ORB detector)
    ...
    \item \href{chrome-extension://efaidnbmnnnibpcajpcglclefindmkaj/https://archive.air.in.tum.de/Main/Publications/Mair2010c.pdf}{AGAST} \cite{mair2010adaptive}
    \item \href{http://cmp.felk.cvut.cz/~matas/papers/matas-bmvc02.pdf}{MSER} \cite{matas2002robust}
    \item \href{https://link.springer.com/chapter/10.1007/978-3-540-88693-8_8}{StarDector/CenSurE} \cite{censure}
    \item \href{https://www.robots.ox.ac.uk/~vgg/research/affine/det_eval_files/mikolajczyk_ijcv2004.pdf}{Harris-Laplace} \cite{mikolajczyk_inria00548554}
    \item \href{https://github.com/MagicLeapResearch/SuperPointPretrainedNetwork}{SuperPoint}\cite{detone18superpoint}
    ...
    \item \href{https://github.com/axelBarroso/Key.Net}{KeyNetAffNetHardNet} (KeyNet detector \cite{barroso2020key} + AffNet + HardNet descriptor \cite{mishchuk2017working})
\end{itemize}
...
\begin{itemize}
    \item \href{http://www.willowgarage.com/sites/default/files/orb_final.pdf}{ORB} \cite{rublee2011orb}
    \item \href{https://www.cs.ubc.ca/~lowe/papers/iccv99.pdf}{SIFT} \cite{lowe1999object}
    \item \href{https://www.robots.ox.ac.uk/~vgg/publications/2012/Arandjelovic12/arandjelovic12.pdf}{ROOT SIFT} \cite{arandjelovic2012three}
    \item \href{http://people.ee.ethz.ch/~surf/eccv06.pdf}{SURF} \cite{bay2006surf}
    \item \href{http://www.bmva.org/bmvc/2013/Papers/paper0013/paper0013.pdf}{AKAZE} \cite{alcantarilla2013fast}
    \item \href{http://www.margaritachli.com/papers/ICCV2011paper.pdf}{BRISK} \cite{leutenegger2011brisk}
    \item \href{https://www.researchgate.net/publication/258848394_FREAK_Fast_retina_keypoint}{FREAK} \cite{alahi2012freak}
    \item \href{https://github.com/MagicLeapResearch/SuperPointPretrainedNetwork}{SuperPoint} \cite{detone18superpoint}
    \item \href{https://github.com/vbalnt/tfeat}{Tfeat} \cite{balntas2016learning}
    ...
    \item \href{https://pdfs.semanticscholar.org/85bd/560cdcbd4f3c24a43678284f485eb2d712d7.pdf}{LUCID} \cite{lucid2015}
    ...
    \item \href{https://github.com/yuruntian/SOSNet.git}{SOSNet} \cite{SOSNet}
    \item \href{https://github.com/yuruntian/L2-Net}{L2Net} \cite{L2Net}
    \item \href{https://github.com/cvlab-epfl/log-polar-descriptors}{Log-polar descriptor} \cite{LogPolarDescriptor}
    \item \href{https://github.com/mihaidusmanu/d2-net}{D2-Net} \cite{dusmanu2019d2}
    \item \href{https://github.com/tensorflow/models/tree/master/research/delf}{DELF} \cite{noh2017large}
    \item \href{https://github.com/lzx551402/contextdesc}{Contextdesc} \cite{luo2020contextdesc}
    \item \href{https://github.com/vcg-uvic/lf-net-release}{LFNet} \cite{ono2018lf}
    \item \href{https://github.com/naver/r2d2}{R2D2} \cite{revaud2019r2d2}
    \item \href{https://raw.githubusercontent.com/iago-suarez/BEBLID/master/BEBLID_Boosted_Efficient_Binary_Local_Image_Descriptor.pdf}{BEBLID} \cite{BEBLID}
    \item \href{https://arxiv.org/abs/2006.13566}{DISK} \cite{tyszkiewicz2020disk}
    \item \href{https://arxiv.org/abs/2304.03608}{ALIKED} \cite{barroso2023alike}
    \item \href{https://arxiv.org/abs/2404.19174}{Xfeat} \cite{barroso2024xfeat}
    \item \href{https://github.com/axelBarroso/Key.Net}{KeyNetAffNetHardNet} (KeyNet detector \cite{barroso2020key} + AffNet + HardNet descriptor \cite{mishchuk2017working})
...
For more information, refer to \href{https://github.com/luigifreda/pyslam/blob/master/pyslam/local_features/feature_types.py}{pyslam/local\_features/feature\_types.py}
...
\href{https://github.com/luigifreda/pyslam/blob/master/pyslam/local_features/feature_tracker_configs.py}{pyslam/local\_features/feature\_tracker.configs.py}

The function \texttt{feature\_tracker\_factory()} can be found in the
file \texttt{pyslam/local\_features/feature\_tracker.py}. Take a look at the
file \texttt{pyslam/local\_features/feature\_manager.py} for further details.
...
\begin{itemize}
    ...
    \item \href{https://arxiv.org/abs/2404.19174}{XFeat} \cite{barroso2024xfeat}
    \item \href{https://arxiv.org/abs/2306.13643}{LightGlue} \cite{lindenberger2023lightglue}
    \item \href{https://arxiv.org/abs/2104.00680}{LoFTR} \cite{sun2021loftr}
    \item \href{https://arxiv.org/abs/2406.09756}{MASt3R} \cite{master}
\end{itemize}

See the file \texttt{pyslam/local\_features/feature\_matcher.py} for further
details.
...
\begin{itemize}
    \item Bag of Words (BoW): \href{https://github.com/dorian3d/DBoW2}{DBoW2} \cite{galvez2012bags}, \href{https://github.com/rmsalinas/DBow3}{DBoW3} \cite{galvez2012bags}.  \href{https://doi.org/10.1109/TRO.2012.2197158}{[paper]}
    \item Vector of Locally Aggregated Descriptors: \href{https://www.vlfeat.org/api/vlad.html}{VLAD} \cite{jegou2010aggregating}.  \href{https://doi.org/10.1109/CVPR.2010.5540039}{[paper]}
    \item Incremental Bags of Binary Words (iBoW) via Online Binary Image Index: \href{https://github.com/emiliofidalgo/ibow-lcd}{iBoW} \cite{garcia2018ibow}, \href{https://github.com/emiliofidalgo/obindex2}{OBIndex2} \cite{garcia2018ibow}.  \href{https://doi.org/10.1109/LRA.2018.2849609}{[paper]}
...
\tightlist
\item
  \href{https://ieeexplore.ieee.org/document/6224623}{SAD} \cite{milford2012seqslam}
\item
  \href{https://github.com/BVLC/caffe/tree/master/models/bvlc_alexnet}{AlexNet} \cite{krizhevsky2012imagenet}
\item
  \href{https://www.di.ens.fr/willow/research/netvlad/}{NetVLAD} \cite{arandjelovic2016netvlad}
\item
  \href{https://www.tu-chemnitz.de/etit/proaut/hdc_desc}{HDC-DELF} \cite{schlegel2022comparison}
\item
...
descriptors. See the file \href{https://github.com/luigifreda/pyslam/blob/master/pyslam/loop_closing/loop_detector_configs.py}{pyslam/loop\_closing/loop\_detector\_configs.py}
for further details.
...
\tightlist
\item
  \href{https://ieeexplore.ieee.org/document/4359315}{SGBM}\cite{hirschmuller2007stereo}: Depth SGBM
  from OpenCV (Stereo, classic approach) 
\item
...
\tightlist
\item
  A C++ template-based voxel grid implementation leverages parallel
  spatial hashing, supporting both direct voxel hashing and indirect
  voxel-block hashing strategies. Parallel execution is managed using
  TBB, and the design accommodates both simple and semantic voxels.
  Further information about the volumetric grid models is available
  \href{https://github.com/luigifreda/pyslam/blob/master/pyslam/cpp/volumetric/README.md}{here}.
\item    
...
\subsection{Supported semantic segmentation
...
\item
  \href{https://arxiv.org/abs/2412.08628v1}{EOV\_SEG}~\cite{eov_seg}: EOV-Seg: Efficient Open-Vocabulary Panoptic Segmentation.
\item
  \href{https://arxiv.org/abs/2201.02605}{DETIC}~\cite{detic}: Detecting Twenty-thousand Classes using Image-level Supervision.
\item
  \href{https://arxiv.org/abs/2303.04803}{ODISE}~\cite{odise}: Open-Vocabulary Panoptic Segmentation with Text-to-Image Diffusion Models.
\item
  \href{https://arxiv.org/abs/2511.09554}{RFDETR}~\cite{rfdetr}: A real-time transformer architecture for object detection and instance segmentation, built on a DINOv2 vision transformer backbone.
\item
  \href{https://arxiv.org/abs/2510.09653}{YOLO}~\cite{sapkota2025ultralytics}: One of the most influential and widely adopted series of models for real-time object detection, that attains an excellent balance
between high accuracy and inference speed.
\end{itemize}
\hypertarget{supported-models-for-feed-forward-inference-of-3d-scenes-from-multiple-images}{%
\subsubsection{Supported models for feed-forward inference of 3D scenes
from multiple
images}\label{supported-models-for-feed-forward-inference-of-3d-scenes-from-multiple-images}}

\begin{itemize}
\tightlist
\item
  \href{https://arxiv.org/abs/2312.14132}{DUSt3R}\cite{wang2023dust3r}: Geometric 3D Vision
  Made Easy
\item
  \href{https://arxiv.org/abs/2406.09756}{MASt3R}\cite{master}: Grounding Image
  Matching in 3D with MASt3R 
\item
  \href{https://arxiv.org/abs/2511.10647}{Depth Anything V3}\cite{depthanything3}: Recovering
  the Visual Space from Any Views
\item
  \href{https://github.com/naver/mvdust3r}{MVDust3r}\cite{mvduster}: Multi-view DUSt3R
  variant
\item
  \href{https://github.com/facebookresearch/vggt}{VGGT}\cite{wang2025vggt}: Visual Geometry
  Grounded Transformer
\item
  \href{https://github.com/cvlab-kaist/RobustVGGT.git}{Robust VGGT}\cite{vggt_robust}:
  Emergent Outlier View Rejection in Visual Geometry Grounded
  Transformers
\item
  \href{https://arxiv.org/abs/2501.13928}{Fast3R}\cite{yang2025fast3r}: Fast3R: Towards 3D Reconstruction of 1000+ Images in One Forward Pass
\end{itemize}

See \href{https://github.com/luigifreda/pyslam/blob/master/pyslam/scene_from_views/README.md}{here} for further details about
\texttt{SceneFromViews} architecture.
...
\href{https://github.com/luigifreda/pyslam/blob/master/config.yaml}{config.yaml} and affect the configuration
parameters in \href{https://github.com/luigifreda/pyslam/blob/master/pyslam/config_parameters.py}{config\_parameters.py}.
...
\endhead
\href{http://www.cvlibs.net/datasets/kitti/eval_odometry.php}{KITTI
odometry data set} \cite{Geiger2012CVPR} &
\texttt{type:\ KITTI\_DATASET} \tabularnewline
\href{https://vision.in.tum.de/data/datasets/rgbd-dataset/download}{TUM
dataset} \cite{sturm12iros} & \texttt{type:\ TUM\_DATASET} \tabularnewline
\href{https://www.doc.ic.ac.uk/~ahanda/VaFRIC/iclnuim.html}{ICL-NUIM
dataset} \cite{handa:etal:ICRA2014} & \texttt{type:\ ICL\_NUIM\_DATASET}\tabularnewline
\href{https://projects.asl.ethz.ch/datasets/euroc-mav/}{EUROC
dataset} \cite{Burri25012016} & \texttt{type:\ EUROC\_DATASET}\tabularnewline
\href{https://github.com/facebookresearch/Replica-Dataset}{REPLICA
dataset} \cite{replica19arxiv} & \texttt{type:\ REPLICA\_DATASET}\tabularnewline
\href{https://theairlab.org/tartanair-dataset/}{TARTANAIR dataset} \cite{tartanair2020iros} &
\texttt{type:\ TARTANAIR\_DATASET}\tabularnewline
\href{https://www.microsoft.com/en-us/research/project/rgb-d-dataset-7-scenes/}{Seven Scenes dataset} \cite{seven_scenes} &
\texttt{type:\ SEVEN\_SCENES\_DATASET}\tabularnewline
\href{https://github.com/dazinovic/neural-rgbd-surface-reconstruction}{Neural RGBD dataset} \cite{Azinovic_2022_CVPR} &
\texttt{type:\ NEURAL\_RGBD}\tabularnewline
\href{https://iis-esslingen.github.io/rover/}{Rover dataset} \cite{schmidt2025rover} &
\texttt{type:\ ROVER\_DATASET}\tabularnewline
\href{http://www.scan-net.org/}{ScanNet dataset} \cite{dai2017scannet} &
\texttt{type:\ SCANNET\_DATASET}\tabularnewline
\href{https://github.com/MIT-SPARK/Clio}{CLIO dataset} \cite{kim2024clio} &
\texttt{type:\ CLIO\_DATASET}\tabularnewline
...
\href{https://foxglove.dev/blog/introducing-the-mcap-file-format}{MCAP
file} & \texttt{type:\ MCAP\_DATASET}\tabularnewline
...
\item
  ...
  \href{http://vision.in.tum.de/data/datasets/rgbd-dataset/download}{vision.in.tum.de/data/datasets/rgbd-dataset/download} and
  uncompress it.
\item
  ...
  \texttt{\$\ python\ associate.py\ PATH\_TO\_SEQUENCE/rgb.txt\ PATH\_TO\_SEQUENCE/depth.txt\ \textgreater{}\ associations.txt\ \ \ \ \ \ \#\ pay\ attention\ to\ the\ order!}
\item
  Select the corresponding calibration settings file (section
...
\item
  ...
  \href{https://theairlab.org/tartanair-dataset}{https://theairlab.org/tartanair-dataset}
\item
...
\hypertarget{scenes-dataset}{%
\paragraph{7 Scenes dataset}\label{scenes-dataset}}

To dowload the dataset follow the instructions at
https://www.microsoft.com/en-us/research/project/rgb-d-dataset-7-scenes/.
Select the section \texttt{SEVEN\_SCENES\_DATASET:\ settings:} in the
file \texttt{config.yaml}.A calibration file
\texttt{settings/SEVEN\_SCENES.yaml} is already available.

\hypertarget{neural-rgbd-dataset}{%
\paragraph{Neural RGBD dataset}\label{neural-rgbd-dataset}}

To dowload the dataset follow the instructions at
https://github.com/dazinovic/neural-rgbd-surface-reconstruction. Select
the section \texttt{NEURAL\_RGBD\_DATASET:\ settings:} in the file
\texttt{config.yaml}. A calibration file
\texttt{settings/NEURAL\_RGBD.yaml} is already available.

\hypertarget{rover-dataset}{%
\paragraph{Rover dataset}\label{rover-dataset}}

To dowload the dataset follow the instructions at
https://iis-esslingen.github.io/rover/. Select the section
\texttt{ROVER\_DATASET:\ settings:} in the file \texttt{config.yaml}. A
calibration file for the \texttt{d435i} camera is already available:
\texttt{settings/ROVER\_d435i.yaml}. Other camera models will be
...
\item
  ...
  \href{http://www.scan-net.org}{http://www.scan-net.org}. You will need to request the dataset from
  the authors.
\item
...
\hypertarget{clio-datasets}{%
\paragraph{CLIO Datasets}\label{clio-datasets}}

pySLAM supports the RGB-D sequences released with
\href{https://github.com/MIT-SPARK/Clio}{Clio} \cite{kim2024clio}
(Hydra/Clio scene-graph mapping). Each scene is expected to follow this
layout under \texttt{\{base\_path\}/\{name\}/}:

\begin{scriptsize}
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{images/rgb\_\{id\}.jpg}
\NormalTok{depth/depth\_\{id\}.png}
\NormalTok{sparse/0/images.bin          \# COLMAP sparse model (optional reference poses)}
\NormalTok{*.bag                        \# ROS1 recording (fps auto-detection, metric odometry)}
\end{Highlighting}
\end{Shaded}
\end{scriptsize}
...
\tightlist
\item
  Download or prepare a CLIO scene and set \texttt{base\_path} and
  \texttt{name} in the \texttt{CLIO\_DATASET} section of
  \texttt{config.yaml}.
\item
  Select the calibration settings file (\texttt{CLIO\_DATASET:\ settings:\
  settings/CLIO.yaml}). Depth maps are stored as \texttt{uint16} in
  \textbf{millimeters} (\texttt{DepthMapFactor:\ 1000.0}).
\item
  Set \texttt{sensor\_type:\ rgbd} (or \texttt{mono} if depth is
  unavailable).
\item
  Set \texttt{groundtruth\_file:\ auto} to load a \textbf{reference
  trajectory} for evaluation and visualization (not independent
  motion-capture GT). By default, pySLAM prefers metric poses from the
  scene ROS1 bag topic \texttt{/dominic/forward/colmap\_odom}; if the
  bag is missing, it falls back to COLMAP \texttt{sparse/0} poses
  (typically up-to-scale). Use \texttt{clio\_reference\_poses:\ auto\ |\ bag\ |\ sparse}
  to override this choice.
\item
  Playback rate is auto-detected from the bag when present
  (${\sim}7.5$~Hz on the office scene); you can override it with
  \texttt{fps} in \texttt{config.yaml} or \texttt{Camera.fps} in the
  settings file.
\end{enumerate}

When the reference trajectory is non-metric (COLMAP sparse), pySLAM
automatically uses Sim(3) alignment for the ground-truth overlay and ATE
metrics.
...
\hypertarget{mcap-file}{%
\paragraph{MCAP file}\label{mcap-file}}

A concise introduction of the MCAP format is available at \href{https://foxglove.dev/blog/introducing-the-mcap-file-format}{https://foxglove.dev/blog/introducing-the-mcap-file-format}. These files can be also recorded and played back by using ROS2 tools.  
...
\begin{itemize}
\item Calibrate it and configure \href{https://github.com/luigifreda/pyslam/blob/master/settings/WEBCAM.yaml}{settings/WEBCAM.yaml} accordingly.
...
A list of the frameworks that inspired or has been integrated into pySLAM are available \href{https://github.com/luigifreda/pyslam?tab=readme-ov-file#credits}{here}. Many thanks to their Authors for their great work.
%
%\begin{itemize}
%\tightlist
%\item
...
%  adding the first version of the trajectory-saving feature and for the comparison notebook:
...
%\item 
%	Many thanks to \href{https://github.com/dvdmc}{David Morilla Cabello} for his great work on creating the first version of sparse semantic mapping module in pySLAM~\cite{pyslamSemantic2025}.
%\end{itemize}
